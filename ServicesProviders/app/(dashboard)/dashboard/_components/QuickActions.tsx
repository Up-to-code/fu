"use client";

import { Button } from "@/components/ui/button";
import { Plus, Settings, MessageSquare, Wallet } from "lucide-react";
import Link from "next/link";

export function QuickActions() {
    return (
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-2">
            <Link href="/services/new">
                <Button variant="outline" className="w-full h-20 items-center justify-start px-6 gap-4 rounded-xl border-gray-200 hover:bg-gray-50 hover:border-primary/20 transition-all group">
                    <div className="h-10 w-10 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-white">
                        <Plus className="h-5 w-5 text-gray-600" />
                    </div>
                    <span className="text-gray-900 font-bold">إضافة خدمة</span>
                </Button>
            </Link>
            <Link href="/messages">
                <Button variant="outline" className="w-full h-20 items-center justify-start px-6 gap-4 rounded-xl border-gray-200 hover:bg-gray-50 hover:border-primary/20 transition-all group">
                    <div className="h-10 w-10 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-white">
                        <MessageSquare className="h-5 w-5 text-gray-600" />
                    </div>
                    <span className="text-gray-900 font-bold">الرسائل</span>
                </Button>
            </Link>
            <Link href="/finance">
                <Button variant="outline" className="w-full h-20 items-center justify-start px-6 gap-4 rounded-xl border-gray-200 hover:bg-gray-50 hover:border-primary/20 transition-all group">
                    <div className="h-10 w-10 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-white">
                        <Wallet className="h-5 w-5 text-gray-600" />
                    </div>
                    <span className="text-gray-900 font-bold">المالية</span>
                </Button>
            </Link>
            <Link href="/settings">
                <Button variant="outline" className="w-full h-20 items-center justify-start px-6 gap-4 rounded-xl border-gray-200 hover:bg-gray-50 hover:border-primary/20 transition-all group">
                    <div className="h-10 w-10 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-white">
                        <Settings className="h-5 w-5 text-gray-600" />
                    </div>
                    <span className="text-gray-900 font-bold">الإعدادات</span>
                </Button>
            </Link>
        </div>
    );
}
